...
func (r *settingsResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	resp.Diagnostics.AddError(
		...
		"This resource manages existing DNSmasq configuration that cannot be created or destroyed.\n\n"+
			"To manage this resource, you must import it first:\n"+
			"  terraform import opnsense_dnsmasq_settings.<name> dnsmasq_settings\n\n"+
...
func (r *settingsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	settings, err := r.client.Dnsmasq().GeneralSettingsGet(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read Dnsmasq settings, got error: %s", err))
		return
	}
	tflog.Info(ctx, "DNSMASQ raw settings", map[string]any{
		"settings": settings,
	})
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to parse Dnsmasq settings, got error: %s", err))
		return
	}

	// Preserve the ID from state (always "dnsmasq_settings")
	resourceModel.Id = data.Id

	tflog.Trace(ctx, "read dnsmasq settings resource")

	// Save updated data into Terraform state
	resp.Diagnostics.Append(resp.State.Set(ctx, &resourceModel)...)
}

// Update modifies the dnsmasq singleton configuration.
func (r *settingsResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	_, err = r.client.Dnsmasq().GeneralSettingsSet(ctx, &resourceStruct.Dnsmasq)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to update dnsmasq settings, got error: %s", err))
		return
	}

	// Reconfigure the service to apply changes
	_, err = r.client.Dnsmasq().ServiceReconfigure(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to reconfigure dnsmasq service, got error: %s", err))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to read updated dnsmasq settings, got error: %s", err))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error",
			fmt.Sprintf("Unable to parse updated dnsmasq settings, got error: %s", err))
		return
	}
	...
	tflog.Trace(ctx, "updated dnsmasq settings resource")
...
func (r *settingsResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Warn(ctx,
		"Singleton resource removed from Terraform state. "+
			"The Dnsmasq configuration remains unchanged and will not be deleted. "+
	...
	resp.Diagnostics.AddWarning(
		...
		"This resource has been removed from Terraform state, but the "+
			"Dnsmasq configuration has NOT been deleted or modified. The settings "+
...
// ImportState imports the singleton resource using the fixed ID "dnsmasq_settings".
func (r *settingsResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	// Validate that the import ID is "dnsmasq_settings"
	if req.ID != "dnsmasq_settings" {
		resp.Diagnostics.AddError(
			"Invalid Import ID",
			"This is a singleton resource and must be imported using the ID 'dnsmasq_settings'.\n\n"+
				"Usage:\n"+
				"  terraform import opnsense_dnsmasq_settings.<name> dnsmasq_settings\n\n"+
	...
	tflog.Info(ctx, "imported dnsmasq settings resource", map[string]any{"id": req.ID})
}
